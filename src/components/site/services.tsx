      ...
      <div className="mx-auto max-w-6xl">
          ...
          {SERVICES.map((service, i) => (
            <Reveal key={service.num} delay={i * 70}>
              <article className="service-card glass-card h-full rounded-2xl p-5 md:p-6">
                <p className="font-display text-sm tracking-[0.18em] text-gold-bright">
                  {service.num}
                </p>
                <h3 className="mt-3 font-display text-xl font-semibold text-gold-bright">
                  {service.title}
                </h3>
        ...
        <Reveal delay={120}>
          <p className="glass-card mt-6 rounded-xl px-5 py-4 text-sm text-muted md:text-base">
            <span className="font-medium text-gold">مش عارف تختار؟</span>{" "}
